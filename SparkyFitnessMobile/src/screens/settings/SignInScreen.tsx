...
type SignInScreenProps = CompositeScreenProps<
  StackScreenProps<SettingsStackParamList, 'SignInSettings'>,
...
const SignInScreen: React.FC<SignInScreenProps> = ({ route, navigation }) => {
  ...
  // Proxy headers lifecycle: set on focus, clear on blur
  useFocusEffect(
    React.useCallback(() => {
        ...
        {step === 'credentials' ? (
          ...
          <>
            ...
            {mfaFactors.mfaTotpEnabled && mfaFactors.mfaEmailEnabled && (
              <View className="flex-row mb-4 mx-4 rounded-lg overflow-hidden border border-border-subtle">
                {([
            ...
            {/* MFA Instructions */}
            <Text className="text-sm text-text-secondary mb-3 text-center px-4">
              {mfaMethod === 'totp'
